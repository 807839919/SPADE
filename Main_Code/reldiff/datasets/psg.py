...
class PanopticSceneGraphDataset(CocoPanopticDataset):
    ...
    def _init_cocoapi(self):
        ...
        thing_categories = [
            {"id": i, "name": name, "isthing": 1}
            for i, name in enumerate(self.THING_CLASSES)
        ]
        stuff_categories = [
            {"id": i + len(self.THING_CLASSES), "name": name, "isthing": 0}
    ...
    def get_ann_info(self, idx):
        ...
        for s in d["segments_info"]:
            gt_mask_infos.append(
                {"id": s["id"], "category": s["category_id"], "is_thing": s["isthing"]}
            )
    ...
    def evaluate(
        ...
        **kwargs,
        ...
        # Available metrics
        allowed_sg_metrics = ["predcls", "sgcls", "sgdet", "pairdet"]
        ...
        if len(sg_metrics) > 0:
            ...
            if not hasattr(self, "test_gt_results"):
                ...
                for i in range(len(self)):
                    ...
                    gt_img = read_image(
                        self.seg_prefix + "/" + ann["seg_map"], format="RGB"
                    )
