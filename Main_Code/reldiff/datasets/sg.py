...
from reldiff.evaluation import sgg_evaluation
from reldiff.models.relation_heads.approaches import Result
...
class SceneGraphDataset(CocoDataset):
    ...
    def _init_cocoapi(self):
        ...
        auxcoco.dataset = {
            ...
            "categories": [
                {"id": i, "name": name} for i, name in enumerate(self.CLASSES)
            ],
            "annotations": anns,
        }
        auxcoco.createIndex()
    ...
    def get_ann_info(self, idx):
        ...
        gt_bboxes = np.array([a["bbox"] for a in d["annotations"]], dtype=np.float32)
        gt_labels = np.array(
        ...
        #             relation_map[int(gt_rels[i, 0]), int(gt_rels[i, 1])] = int(
        #                 gt_rels[i, 2]
        #             )
        #     else:
        #         relation_map[int(gt_rels[i, 0]), int(gt_rels[i, 1])] = int(
        ...
        **kwargs: contain the paramteters specifically for OD, e.g., proposal_nums.
        ...
        if len(od_metrics) > 0:
            ...
            if not isinstance(results[0], Result):
                # it may be the results from non-SGG model (like purely object detection)
                od_results = results
            else:
                results = [[r.refine_bboxes, r.labels] for r in results]
                od_results = [
                    bbox2result(det_bboxes, det_labels, len(self.CLASSES))
        ...
        if len(sg_metrics) > 0:
            ...
            prepare the ground truth and predictions. Transform the predictions
