...
from reldiff.models.relation_heads.approaches import PointNetFeat
from reldiff.utils import enumerate_by_image
...
class VisualSpatialExtractor(BaseModule):
    ...
    def single_roi_forward(
        self, feats, rois, masks=None, points=None, roi_scale_factor=None
    ):
        roi_feats_bbox, roi_feats_mask, roi_feats_point = None, None, None
        # 1. Use the visual and spatial head to extract roi features.
        if self.with_visual_bbox:
            roi_feats_bbox = self.roi_forward(
                self.bbox_roi_layers, feats, rois, masks, roi_scale_factor
            )
        if self.with_visual_mask:
            roi_feats_mask = self.roi_forward(
                self.mask_roi_layers, feats, rois, masks, roi_scale_factor
            )
        if self.with_visual_point:
        ...
        if self.num_visual_head > 1:
            if self.gather_visual == "cat":
                ...
                raise NotImplementedError(
                    "The gathering operation {} is not implemented yet.".format(
                        self.gather_visual
                    )
                )
    ...
    def union_roi_forward(
        ...
        if masks is not None and self.with_visual_mask:
            ...
            for i, pair_idx in enumerate(rel_pair_index.cpu().numpy()):
                ...
                union_mask = torch.zeros(union_heights[i], union_widths[i]).to(
                    head_mask
                )
                base_x, base_y = union_rois_int[i, 1], union_rois_int[i, 2]
                union_mask[
                    (head_rois_int[i, 2] - base_y) : (head_rois_int[i, 4] - base_y + 1),
                    (head_rois_int[i, 1] - base_x) : (head_rois_int[i, 3] - base_x + 1),
                ] = head_mask
                union_mask[
                    (tail_rois_int[i, 2] - base_y) : (tail_rois_int[i, 4] - base_y + 1),
                    (tail_rois_int[i, 1] - base_x) : (tail_rois_int[i, 3] - base_x + 1),
                ] = tail_mask
        ...
        if self.with_visual_bbox:
            roi_feats_bbox = self.roi_forward(
                self.bbox_roi_layers, feats, union_rois, union_masks, roi_scale_factor
            )
        if self.with_visual_mask:
            roi_feats_mask = self.roi_forward(
                self.mask_roi_layers, feats, union_rois, union_masks, roi_scale_factor
            )
        if self.with_visual_point:
            roi_feats_point, trans_matrix, _ = self.pointFeatExtractor(
        ...
        dummy_x_range = (
            torch.arange(self.spatial_size)
            .to(rel_pair_index.device)
            .view(1, 1, -1)
            .expand(num_rel, self.spatial_size, self.spatial_size)
        )
        dummy_y_range = (
            torch.arange(self.spatial_size)
            .to(rel_pair_index.device)
        ...
        size_list = [
            np.array(img_meta["img_shape"][:2]).reshape(1, -1) for img_meta in img_metas
        ]
        ...
        head_rect = (
            (dummy_x_range >= head_proposals[:, 1].floor().view(-1, 1, 1).long())
            & (dummy_x_range <= head_proposals[:, 3].ceil().view(-1, 1, 1).long())
            & (dummy_y_range >= head_proposals[:, 2].floor().view(-1, 1, 1).long())
            & (dummy_y_range <= head_proposals[:, 4].ceil().view(-1, 1, 1).long())
        ).float()
        tail_rect = (
            (dummy_x_range >= tail_proposals[:, 1].floor().view(-1, 1, 1).long())
            & (dummy_x_range <= tail_proposals[:, 2].ceil().view(-1, 1, 1).long())
            & (dummy_y_range >= tail_proposals[:, 3].floor().view(-1, 1, 1).long())
            & (dummy_y_range <= tail_proposals[:, 4].ceil().view(-1, 1, 1).long())
        ).float()

        rect_input = torch.stack(
            (head_rect, tail_rect), dim=1
        )  # (num_rel, 2, rect_size, rect_size)
            ...
            for roi_feats, head in (
                ...
                if head is not None:
                    roi_feats_result.append(
                        head((roi_feats + rect_feats).view(roi_feats.size(0), -1))
                    )
            if self.num_visual_head > 1:
                if self.gather_visual == "cat":
                    roi_feats_result = torch.cat(roi_feats_result, dim=-1)
    ...
    def forward(
        ...
        if self.with_spatial:
            ...
            return self.union_roi_forward(
                feats, img_metas, rois, rel_pair_idx, masks, points, roi_scale_factor
            )
        else:
