...
class PSGTrHead(AnchorFreeHead):
    ...
    def __init__(
        ...
        config_path = "/home/user/huxin/Pair-Net-main/StableDiffusion/checkpoints/v1-inference.yaml"
        ckpt_path = "/home/user/huxin/Pair-Net-main/StableDiffusion/checkpoints/v1-5-pruned-emaonly.ckpt"
        ...
        _, self.clip_preprocess = clip.load("ViT-B/32", device=device)
    ...
    def forward(self, feats, img_metas):
        ...
        # Load the image and create CLIP and SD inputs
        clip_inputs_list = []
        sd_inputs_list = []
        for meta in img_metas:
            ...
            # Preprocess for CLIP
            clip_input = self.clip_preprocess(img)
            clip_inputs_list.append(clip_input.unsqueeze(0))  # Unsqueeze for batch dimension

            # Preprocess for SD (resize to 512x512 for SD input)
            transform = transforms.ToTensor()
            sd_input = torch.nn.functional.interpolate(transform(img).unsqueeze(0), size=(512, 512))
            sd_inputs_list.append(sd_input.squeeze(0))

        # Stack the inputs
        clip_inputs = torch.cat(clip_inputs_list, dim=0).to(device)  # 将输入移动到设备上
        sd_inputs = torch.stack(sd_inputs_list).to(device)  # 将输入移动到设备上

        # Add CLIP and SD inputs to img_metas
        for i, meta in enumerate(img_metas):
            meta['clip_inputs'] = clip_inputs[i]
            meta['sd_inputs'] = sd_inputs[i]



        sd_feat = self.extract_feat(last_features, img_metas)
        ...
        outs_dec, memory = self.transformer(
            sd_decoder, masks, self.query_embed.weight, pos_embed
        )
        ...
        if self.use_mask:
            ###########for segmentation#################
            sub_bbox_mask = self.sub_bbox_attention(outs_dec[-1], memory, mask=masks)
            obj_bbox_mask = self.obj_bbox_attention(outs_dec[-1], memory, mask=masks)
            sub_seg_masks = self.sub_mask_head(
                last_features, sub_bbox_mask, [feats[2], feats[1], feats[0]]
            )
            outputs_sub_seg_masks = sub_seg_masks.view(
                batch_size,
                self.num_query,
                sub_seg_masks.shape[-2],
                sub_seg_masks.shape[-1],
            )
            obj_seg_masks = self.obj_mask_head(
                last_features, obj_bbox_mask, [feats[2], feats[1], feats[0]]
            )
            outputs_obj_seg_masks = obj_seg_masks.view(
                batch_size,
                self.num_query,
