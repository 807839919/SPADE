...
class DecoderRNN(nn.Module):
    def __init__(
        ...
        obj_embed_vecs = obj_edge_vectors(
            ["start"] + self.obj_classes, wv_dir=self.cfg.glove_dir, wv_dim=embed_dim
        )
        ...
        self.input_linearity = torch.nn.Linear(
            self.input_size, 6 * self.hidden_size, bias=True
        )
        self.state_linearity = torch.nn.Linear(
            self.hidden_size, 5 * self.hidden_size, bias=True
        )
    ...
    def init_weights(self):
        ...
        self.input_linearity.bias.data[self.hidden_size : 2 * self.hidden_size].fill_(
            1.0
        )

    def lstm_equations(
        self, timestep_input, previous_state, previous_memory, dropout_mask=None
    ):
    ...
    def forward(self, inputs, initial_state=None, labels=None, boxes_for_nms=None):
        ...
        if initial_state is None:
            previous_memory = (
                sequence_tensor.new().resize_(batch_size, self.hidden_size).fill_(0)
            )
            previous_state = (
                sequence_tensor.new().resize_(batch_size, self.hidden_size).fill_(0)
            )
        ...
        if self.rnn_drop > 0.0:
            dropout_mask = get_dropout_mask(
                self.rnn_drop, previous_memory.size(), previous_memory.device
            )
        else:
            dropout_mask = None
        ...
        for i, l_batch in enumerate(batch_lengths):
            ...
            if self.training:
                ...
                # Whenever labels are 0 set input to be spade max prediction
...
class LSTMContext(nn.Module):
    ...
    def __init__(self, config, obj_classes, rel_classes):
        ...
        if self.effect_analysis:
            ...
            self.register_buffer(
                "untreated_obj_feat", torch.zeros(self.obj_dim + self.embed_dim + 128)
            )
            self.register_buffer(
                "untreated_edg_feat", torch.zeros(self.embed_dim + self.obj_dim)
            )
    ...
    def forward(self, x, det_result, all_average=False, ctx_average=False):
        ...
        if (
            self.training or self.use_gt_box
        ...
        if (
            all_average and self.effect_analysis and (not self.training)
        ):  # TDE: only in test mode
            obj_pre_rep = self.untreated_obj_feat.view(1, -1).expand(batch_size, -1)
        else:
            obj_pre_rep = torch.cat(
                (x, obj_embed, pos_embed), -1
            )  # N x (1024 + 200 + 128)
        ...
        if (
            (all_average or ctx_average)
            and self.effect_analysis
            and (not self.training)
        ):  # TDE: Testing
            obj_rel_rep = torch.cat(
                (self.untreated_edg_feat.view(1, -1).expand(batch_size, -1), obj_ctx),
                dim=-1,
            )
        else:
        ...
        edge_ctx = self.edge_ctx(
            obj_rel_rep, perm=perm, inv_perm=inv_perm, ls_transposed=ls_transposed
        )
